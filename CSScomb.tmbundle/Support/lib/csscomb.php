...
[
	...
	private function parse_selector_rules($string){
		...
		$string = preg_replace('#/\*(.*?)\*/#smi', '', $string); // избавляемся от комментариев в рамках одного селектора внутри фигурных скобок
		...
		$lines = explode("\n", $string);
		if(count($lines)>1) $last = $lines[count($lines)-1]; else $last = ''; // сохраняем форматирование после последнего \n если свойства были по одному на строке, а не div {display: inline-block; *display: inline;}
